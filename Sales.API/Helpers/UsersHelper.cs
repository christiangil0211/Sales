namespace Sales.API.Helpers
{
    ...
    public class UsersHelper : IDisposable
    {
        ...
        private static void CreateUserASP(string email, string roleName)
        {
            var userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(userContext));
            var userASP = userManager.FindByEmail(email);
            if (userASP == null)
            ...
            userManager.AddToRole(userASP.Id, email);
        }
        ...
        public static async Task PasswordRecovery(string email)
        {
            ...
            {
                var subject = "Sales App - Recuperacion contraseña";
                var body = string.Format(@"
                <h1>Sales App - Recuperacion de contraseña</h1>
                <p>Su nueva contraseña es: <strong>{0}</strong></p>
                <p>Por favor no olvide cambiarla por una de facil recordacion", newPassword);
